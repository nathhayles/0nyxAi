...
const SOCIAL = [
  { id: "instagram", label: "Instagram",      icon: "📸", color: "#E1306C", status: "connect" },
  { id: "tiktok",    label: "TikTok",         icon: "🎵", color: "#69C9D0", status: "connect" },
  { id: "facebook",  label: "Facebook Pages",  icon: "f",  color: "#1877F2", status: "connect" },
  { id: "twitter",   label: "X (Twitter)",    icon: "𝕏",  color: "#e2e8f0", status: "coming"  },
  { id: "linkedin",  label: "LinkedIn",       icon: "in", color: "#0A66C2", status: "connect" },
];

const SERVICE_ICONS = [
  { label: "Claude AI",       icon: "✦",  color: "#cc785c" },
  { label: "OpenAI TTS",      icon: "🔊", color: "#10a37f" },
  { label: "ElevenLabs",      icon: "🎙", color: "#f97316" },
  { label: "HeyGen Avatars",  icon: "🧑", color: "#6366f1" },
  { label: "Kling AI Video",  icon: "🎬", color: "#ec4899" },
  { label: "Google Lyria",    icon: "🎵", color: "#4285f4" },
  { label: "Pexels Stock",    icon: "📷", color: "#05a081" },
  { label: "Freepik AI",      icon: "🖼", color: "#ff5722" },
  { label: "Supabase",        icon: "⚡", color: "#3ecf8e" },
  { label: "Stripe Billing",  icon: "💳", color: "#6772e5" },
];

const PLAN_COLORS = { pro: "#f59e0b", agency: "#f59e0b", creator: "#8b5cf6", starter: "#2563eb", free: "#475569" };

function StatCard({ title, value, sub, accent = "#2563eb", icon }) {
  return (
    <div style={{ background: "linear-gradient(135deg, #0f141b 0%, #111827 100%)", border: "1px solid #1f2937", borderRadius: 12, padding: "20px 24px", position: "relative", overflow: "hidden" }}>
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 2, background: accent }} />
      <div style={{ fontSize: 24, marginBottom: 8 }}>{icon}</div>
      <div style={{ fontSize: 11, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "1px", marginBottom: 6 }}>{title}</div>
...
function UsageBar({ used, limit, color = "#2563eb" }) {
...
export default function Account() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [creditsRes, voRes, planRes, meRes, reelsRes, brandsRes] = await Promise.all([
      ...
      if (creditsRes.data) setCredits(creditsRes.data.balance || 0);
  ...
  if (loading) return <div style={{ padding: 40, color: "#94a3b8", textAlign: "center", fontSize: 14 }}>Loading account...</div>;
  ...
  const planColor = isTrial ? "#8b5cf6" : trialExpired ? "#ef4444" : PLAN_COLORS[plan] || "#475569";
  const selectedBrand = brands.find(b => b.id === selectedBrandId);
  const atLimit = brands.length >= brandLimit;

  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "32px 24px", color: "#e2e8f0" }}>

      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 32 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 700, margin: 0, marginBottom: 4 }}>Account</h1>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          ...
          <button onClick={handleManageBilling} style={{ padding: "8px 16px", fontSize: 12, fontWeight: 600, background: "#1f2937", border: "1px solid #4b5563", color: "#f1f5f9", borderRadius: 8, cursor: "pointer" }}>Manage Billing →</button>
        </div>
      </div>

      {(isTrial || trialExpired) && (
        <div style={{ marginBottom: 20, padding: "14px 20px", borderRadius: 10, background: isTrial ? "linear-gradient(135deg, rgba(139,92,246,0.12), rgba(99,102,241,0.08))" : "linear-gradient(135deg, rgba(239,68,68,0.12), rgba(220,38,38,0.08))", border: `1px solid ${isTrial ? "rgba(139,92,246,0.3)" : "rgba(239,68,68,0.3)"}`, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
          <div>
          ...
          <button onClick={() => window.location.href = "/pricing"} style={{ padding: "9px 18px", borderRadius: 8, border: "none", whiteSpace: "nowrap", background: "linear-gradient(135deg, #7c3aed, #1d4ed8)", color: "#fff", fontWeight: 700, fontSize: 12, cursor: "pointer", flexShrink: 0 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16, marginBottom: 32 }}>
        <StatCard title="AI Credits" value={credits.toLocaleString()} sub="Used for avatars, AI video & premium voices" icon="⚡" accent="#f59e0b" />
        <StatCard title="Reels Created" value={reelCount} sub="Total projects in your library" icon="🎬" accent="#8b5cf6" />
      ...
      <div style={{ background: "#0f141b", border: "1px solid #1f2937", borderRadius: 12, padding: 24, marginBottom: 24 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: "#94a3b8" }}>Standard voiceover minutes</div>
            <UsageBar used={voUsed} limit={voLimit} color="#10a37f" />
            <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 4 }}>Resets monthly · Free with your plan</div>
          </div>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
              <span>⚡</span>
            ...
            <div style={{ fontSize: 12, color: "#94a3b8" }}>Used across all premium services</div>
              ...
              {[
                ...
                <div key={s.label} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <span style={{ fontSize: 10, color: "#94a3b8", marginLeft: 6 }}>{s.detail}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div style={{ marginTop: 20, padding: "12px 16px", background: "#111827", borderRadius: 8, border: "1px solid #1f2937", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <div style={{ fontSize: 13, fontWeight: 600, color: "#e2e8f0" }}>Need more credits?</div>
            <div style={{ fontSize: 11, color: "#94a3b8" }}>100 credits = $1 · Never expire</div>
          </div>
          <button onClick={() => window.location.href = "/pricing"} style={{ padding: "8px 18px", background: "#1d4ed8", border: "1px solid #1e40af", color: "#fff", borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>Top Up Credits</button>
        </div>
      </div>
      ...
      <div style={{ background: "#0f141b", border: "1px solid #1f2937", borderRadius: 12, padding: 24, marginBottom: 24 }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: "#94a3b8" }}>Each brand has its own social accounts. Select a brand below to manage its connections.</div>
          </div>
        ...
        {!canAutopost && (
          <div style={{ padding: "16px 20px", borderRadius: 10, background: "linear-gradient(135deg, rgba(139,92,246,0.12), rgba(29,78,216,0.12))", border: "1px solid rgba(139,92,246,0.3)", marginBottom: 20, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
            <div>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#e2e8f0", marginBottom: 4 }}>🚀 Unlock Auto-Posting</div>
              <div style={{ fontSize: 12, color: "#94a3b8" }}>
            ...
            <button onClick={handleManageBilling} style={{ padding: "9px 18px", borderRadius: 8, border: "none", whiteSpace: "nowrap", background: "linear-gradient(135deg, #7c3aed, #1d4ed8)", color: "#fff", fontWeight: 700, fontSize: 12, cursor: "pointer", flexShrink: 0 }}>
        ...
        {brands.length > 0 && (
          <div style={{ marginBottom: 20 }}>
            <div style={{ fontSize: 11, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "1px", marginBottom: 8, fontWeight: 700 }}>Select Brand</div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              ...
              {!atLimit
                ? <><button onClick={() => window.location.href = "/branding"} style={{ padding: "7px 16px", borderRadius: 20, fontSize: 13, fontWeight: 600, cursor: "pointer", border: "1px dashed #2b3442", background: "transparent", color: "#475569" }}>+ Add Brand</button><button onClick={() => window.location.href = "/branding"} style={{ padding: "7px 16px", borderRadius: 20, fontSize: 13, fontWeight: 600, cursor: "pointer", border: "1px solid #2b3442", background: "transparent", color: "#94a3b8" }}>Manage Brands →</button></>
                : <button onClick={handleManageBilling} style={{ padding: "7px 16px", borderRadius: 20, fontSize: 13, fontWeight: 600, cursor: "pointer", border: "1px dashed #7c3aed", background: "transparent", color: "#7c3aed" }}>+ Upgrade for more brands</button>
              }
            </div>
            <div style={{ fontSize: 11, color: "#475569", marginTop: 8 }}>{brands.length} / {brandLimit === Infinity ? "∞" : brandLimit} brands on {planLabel} plan</div>
          </div>
        )}
        ...
        {fbPendingPages.length > 0 && (
          <div style={{ marginBottom: 20, padding: "16px 20px", borderRadius: 10, background: "linear-gradient(135deg, rgba(24,119,242,0.12), rgba(29,78,216,0.08))", border: "1px solid rgba(24,119,242,0.35)" }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: "#e2e8f0", marginBottom: 4 }}>Select a Facebook Page to connect</div>
            <div style={{ fontSize: 12, color: "#94a3b8", marginBottom: 12 }}>Your account manages {fbPendingPages.length} pages. Choose one to use for auto-posting.</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {fbPendingPages.map(p => (
                <div key={p.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 14px", borderRadius: 8, background: "#111827", border: "1px solid #1f2937" }}>
                  <div>
            ...
            <div style={{ marginTop: 10, fontSize: 11, color: "#475569" }}>You can change this later by disconnecting and reconnecting.</div>
          </div>
        )}
        ...
        {selectedBrand && (
          <>
            <div style={{ fontSize: 12, color: "#64748b", marginBottom: 12 }}>
              Connections for <span style={{ color: "#a78bfa", fontWeight: 600 }}>{selectedBrand.brand_label}</span>
              {loadingSocial && <span style={{ marginLeft: 8, color: "#475569" }}>Loading...</span>}
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
              {SOCIAL.map(s => {
                ...
                return (
                  <div key={s.id} style={{ padding: "14px 16px", borderRadius: 8, background: "#111827", border: `1px solid ${connected ? s.color + "40" : "#1f2937"}`, display: "flex", alignItems: "center", gap: 10, opacity: !canAutopost && s.status === "connect" ? 0.6 : 1 }}>
                    <div style={{ width: 36, height: 36, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", background: connected ? s.color + "20" : "#1f2937", color: connected ? s.color : "#475569", fontSize: 16, fontWeight: 700, flexShrink: 0 }}>{s.icon}</div>
                    ...
                    {s.status === "connect" && !connected && !canAutopost && (
                      <button onClick={handleManageBilling} style={{ padding: "5px 12px", borderRadius: 6, border: "1px solid #7c3aed40", background: "rgba(124,58,237,0.1)", color: "#a78bfa", fontSize: 11, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0 }}>Upgrade</button>
                    )}
                    {connected && (
                      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, flexShrink: 0 }}>
                        <div style={{ width: 8, height: 8, borderRadius: "50%", background: "#22c55e" }} />
                        <button onClick={() => handleDisconnect(s.id)} disabled={isDisconnecting} style={{ padding: "3px 8px", borderRadius: 4, border: "1px solid #374151", background: "transparent", color: "#475569", fontSize: 10, cursor: "pointer" }}>{isDisconnecting ? "..." : "Disconnect"}</button>
                      </div>
                    )}
                  </div>
                );
        ...
        {brands.length === 0 && (
          <div style={{ textAlign: "center", padding: "24px 0", color: "#475569", fontSize: 13 }}>
            ...
            <div>Set up your first brand in <a href="/branding" style={{ color: "#7c3aed" }}>Branding settings</a> to connect social accounts.</div>
          </div>
        )}
      </div>
      ...
      <div style={{ background: "#0f141b", border: "1px solid #1f2937", borderRadius: 12, padding: 24, marginBottom: 24 }}>
        ...
        <div style={{ fontSize: 12, color: "#94a3b8", marginBottom: 16 }}>Connect your YouTube account to publish reels directly from the editor.</div>
        {ytToken && <YouTubeConnect token={ytToken} />}
      </div>
      ...
      <div style={{ background: "#0f141b", border: "1px solid #1f2937", borderRadius: 12, padding: 24 }}>
        <h2 style={{ fontSize: 16, fontWeight: 700, margin: "0 0 16px" }}>Powered By</h2>
        <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
          {SERVICE_ICONS.map(s => (
            <div key={s.label} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 14px", borderRadius: 8, background: "#111827", border: "1px solid #1f2937" }}>
