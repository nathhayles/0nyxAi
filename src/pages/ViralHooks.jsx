...
export default function ViralHooks() {
  ...
  const s = { minHeight: "100vh", background: "#06070a", color: "#fff", fontFamily: "sans-serif", padding: "40px 24px" };
  const card = { background: "#0c1016", border: "1px solid #1f2937", borderRadius: 12, padding: 20, marginBottom: 16 };
  const inputS = { width: "100%", background: "#0f141b", border: "1px solid #2b3442", color: "#e2e8f0", borderRadius: 8, padding: "10px 14px", fontSize: 14, boxSizing: "border-box", outline: "none" };
  const chipBase = { padding: "7px 14px", borderRadius: 20, fontSize: 12, fontWeight: 600, cursor: "pointer", border: "1px solid #2b3442" };
    ...
    <div style={s}>
      <div style={{ maxWidth: 800, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 28 }}>
          <h1 style={{ fontSize: 28, fontWeight: 700, marginBottom: 8 }}>🔥 Viral Hooks Generator</h1>
        ...
        <div style={card}>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div>
              <label style={{ fontSize: 12, color: "#94a3b8", display: "block", marginBottom: 6 }}>Topic / Subject *</label>
              <input style={inputS} value={topic} onChange={e => setTopic(e.target.value)}
                onKeyDown={e => e.key === "Enter" && generateHooks()}
                placeholder="e.g. how I grew my Instagram to 10k followers in 30 days" />
            </div>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {PLATFORMS.map(p => (
                  <button key={p} onClick={() => setPlatform(p)} style={{
                    ...chipBase,
                    background: platform === p ? "#1e3a5f" : "#111827",
                    border: platform === p ? "1px solid #3b82f6" : "1px solid #1f2937",
                    color: platform === p ? "#60a5fa" : "#64748b",
                    textTransform: "capitalize",
                  }}>{p === "tiktok" ? "🎵" : p === "instagram" ? "📸" : p === "youtube" ? "▶️" : p === "linkedin" ? "💼" : "🐦"} {p}</button>
                ))}
              </div>
            </div>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {TONES.map(t => (
                  <button key={t} onClick={() => setTone(t)} style={{
                    ...chipBase,
                    background: tone === t ? "#1e1b4b" : "#111827",
                    border: tone === t ? "1px solid #7c3aed" : "1px solid #1f2937",
                    color: tone === t ? "#a78bfa" : "#64748b",
                    textTransform: "capitalize",
                  }}>{t}</button>
                ))}
              </div>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "flex-end" }}>
              ...
              <button onClick={generateHooks} disabled={loading} style={{
        ...
        {hooks.length > 0 && (
          <div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
              ...
              <button onClick={() => copyText(hooks.map(h => h.text).join("\n\n"), "all")} style={{ fontSize: 12, color: "#64748b", background: "none", border: "1px solid #2b3442", borderRadius: 6, padding: "5px 12px", cursor: "pointer" }}>
                ...
                <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                      <span style={{ fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 4, background: "rgba(124,58,237,0.15)", color: "#a78bfa" }}>
                    ...
                    {expandedId === i && captions[i] && (
                      <div style={{ marginTop: 12, padding: 14, background: "#111827", borderRadius: 8, border: "1px solid #1f2937" }}>
                        ...
                        <button onClick={() => copyText(`${hook.text}\n\n${captions[i].caption}\n\n${captions[i].hashtags?.map(h => `#${h}`).join(" ")}`, `caption_${i}`)}
                          style={{ marginTop: 10, padding: "6px 14px", borderRadius: 6, background: "#1d4ed8", border: "none", color: "#fff", fontSize: 12, cursor: "pointer", fontWeight: 600 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6, flexShrink: 0 }}>
                    <button onClick={() => copyText(hook.text, i)} style={{ padding: "6px 12px", borderRadius: 6, fontSize: 12, fontWeight: 600, cursor: "pointer", background: copiedId === i ? "rgba(34,197,94,0.15)" : "#1f2937", border: copiedId === i ? "1px solid #22c55e" : "1px solid #2b3442", color: copiedId === i ? "#4ade80" : "#94a3b8" }}>
                      {copiedId === i ? "✅" : "Copy"}
                    </button>
                    <button onClick={() => generateCaption(hook, i)} disabled={captionLoading === i} style={{ padding: "6px 12px", borderRadius: 6, fontSize: 12, fontWeight: 600, cursor: "pointer", background: "#1e1b4b", border: "1px solid #7c3aed", color: "#a78bfa" }}>
                      {captionLoading === i ? "..." : "Caption"}
                    </button>
                    <button onClick={() => openInEditor(hook)} style={{ padding: "6px 12px", borderRadius: 6, fontSize: 12, fontWeight: 600, cursor: "pointer", background: "#1e3a5f", border: "1px solid #3b82f6", color: "#60a5fa" }}>
                      Edit →
                    </button>
                  </div>
                </div>
